...
#define DEFINE_PARAMS(name, enumerators, types)                                                     \
    ...
    std::map<std::string, name> ParamNameReverseLookupMap =                                         \
    ...
    map_type ParamMap = boost::assign::list_of< pair_type >                                         \
    ...
    {     /*param enum value to string*/                                                            \
        ...
        {                                                                                           \
            ...
            default: return "[Unknown " BOOST_PP_STRINGIZE(name) "]";                               \
                                                                                                    ...
                                                                                                    \
                                                                                                    \
    std::vector<std::string> ParamNames = boost::assign::list_of< std::string >                     \
        BOOST_PP_SEQ_FOR_EACH(X_DEFINE_VECTOR_FROM_ENUM, name, enumerators);                        \
    std::vector<rclcpp::ParameterType> ParamTypes = boost::assign::list_of< rclcpp::ParameterType > \
    ...
    inline std::map<name, rclcpp::Parameter>& load_parameters(rclcpp::Node* n) {                    \
        auto type_it = ParamTypes.begin();                                                          \
        for (auto it = ParamNames.begin(); it < ParamNames.end(); ) {                               \
            n->declare_parameter((*it), (*type_it));                                                \
            it++; type_it++;                                                                        \
        }                                                                                           \
        std::vector<rclcpp::Parameter> params = n->get_parameters(ParamNames);                      \
        for (const rclcpp::Parameter& p : params) {                                                 \
            ParamMap[ParamNameReverseLookupMap[p.get_name()]] = p;                                  \
        }                                                                                           \
                                                                                                    \
        return ParamMap;                                                                            \
    }
